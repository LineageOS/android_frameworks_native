...
#ifdef NV_ANDROID_FRAMEWORK_ENHANCEMENTS
        if (dispatchEntry->hasForegroundTarget()) {
            InputHook::getInstance()->notifyKeyState(keyEntry->deviceId, keyEntry->keyCode,
                                                     handled);
        }
#endif
...
#ifdef NV_ANDROID_FRAMEWORK_ENHANCEMENTS
        if (dispatchEntry->hasForegroundTarget()) {
            InputHook::getInstance()->notifyMotionState(motionEntry->deviceId,
                                                        motionEntry->pointerCoords, handled);
        }
#endif
    } else {
        restartEvent = false;
