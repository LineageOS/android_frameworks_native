...
void InputHook::registerDevice(const vendor::nvidia::hardware::shieldtech::inputflinger::V2_0::NewDevice& device) {
    ...
    EventHub::Device* newDevice = new EventHub::Device(dup(handle->data[0]), device.id, String8(device.path.c_str()), *identifier);
...
InputDeviceIdentifier* InputHook::getInputDeviceIdentifier(const InputIdentifier& ident) {
    ...
    identifier->name.setTo(ident.name.c_str());
    identifier->uniqueId.setTo(ident.uniqueId.c_str());
    return identifier;
}
