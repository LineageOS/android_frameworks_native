...
public:
    InputHookCallbackImpl(InputHook* inputHook) { mInputHook = inputHook; }

    Return<void> registerDevice(
            const vendor::nvidia::hardware::shieldtech::inputflinger::V2_0::NewDevice& device) {
...
InputHook::InputHook(EventHub* eventHub) {
    ...
    mRemoteHook =
            vendor::nvidia::hardware::shieldtech::inputflinger::V2_0::IInputHook::getService();
    if (mRemoteHook == NULL) {
...
// Filters
bool InputHook::filterNewDevice(int fd, int32_t id, const String8& path,
    ...
    if (mRemoteHook != NULL) {
        ...
        hardware::Return<void> ret = mRemoteHook->filterNewDevice(nativeHandle, id, path.string(),
                                                                  ident, filterNewDevice_response);
        if (ret.isOk()) {
...
void filterEvent_response(Response response, int32_t replacementDeviceId,
                          const HidlInputEvent& replacementEvent) {
    sResponse = response;
...
void InputHook::registerDevice(
    ...
    EventHub::Device* newDevice =
            new EventHub::Device(dup(handle->data[0]), device.id, device.path, *identifier);
...
status_t InputHook::getAbsoluteAxisInfo(int32_t deviceId, int axis,
                                        RawAbsoluteAxisInfo* outAxisInfo) {
    if (deviceId == -2) {
        outAxisInfo->clear();
        switch (axis) {
            case ABS_X:
