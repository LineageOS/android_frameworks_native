...
static void sa1_handler(int sig) {
    if (sig == SIGIO) {
        ALOGI("signal [%d]", gettid());
        if (main_tid != gettid() && pthread_mutex_lock(&ioMutex)) {
            ALOGI("Error locking mutex: deadlock in signal handler [%d]!", gettid());
        }
        ALOGI("signal [%d] broadcast", gettid());
        pthread_cond_broadcast(&ioWait);
        ALOGI("signal [%d] unlock", gettid());
        if (main_tid != gettid() && pthread_mutex_unlock(&ioMutex)) {
            ALOGI("Error unlocking mutex: signal handler tried to unlock it twice!");
        }
    }
}
...
int main(const int argc, const char *argv[]) {
    ...
    memset (&my_sigaction, 0, sizeof(sigaction));
    my_sigaction.sa_handler = sa1_handler;
    int sigres = sigaction(SIGIO, &my_sigaction, 0);
    ALOGI("Signal registration result: %d", sigres);
