        ...
        uint32_t flags)
{
    ...
    if (mStatus == NO_ERROR) {
        ...
        status_t err = mClient->createSurface(name, w, h, format, flags,
                &handle, &gbp);
        ALOGE_IF(err, "SurfaceComposerClient::createSurface(%s) error %s", name, strerror(-err));
        if (err == NO_ERROR) {
            sur = new SurfaceControl(this, handle, gbp);
        }
    } else
        ALOGE("SurfaceComposerClient::createSurface(%s) error. Status = %s", name, strerror(-mStatus));
    return sur;
}
