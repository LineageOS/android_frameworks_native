...
MemoryHeapIon::MemoryHeapIon(size_t size, uint32_t flags, __attribute__((unused))char const *name):MemoryHeapBase()
{
    void* base = NULL;
    int fd = -1;
...
MemoryHeapIon::MemoryHeapIon(int fd, size_t size, uint32_t flags, __attribute__((unused))uint32_t offset):MemoryHeapBase()
{
    void* base = NULL;
    int dup_fd = -1;
